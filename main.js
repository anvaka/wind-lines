...
function drawCoastline(data) {
  ...
  loadWindMap('2018021000.png').then(drawField);
}

function drawField(result) {
  var uMax = 25.7396; var uMin = -20.9104;
  var vMax = 22.3119; var vMin =   -26.0981;
  var maxVelocity = Math.sqrt(uMax * uMax + vMax * vMax);

  var workingSet = [];
  var animationQueue = []; 
  var maxAnimations = 20, pointsPerAnimation = 42;
  ...
  function vectorField(p) {
    ...
    var lx = Math.floor(p.x); var ly = Math.floor(p.y); 
    var ux = Math.ceil(p.x); var uy = Math.ceil(p.y);
    
    if (lx < 0) lx = ux; if (ux >= result.width) ux = lx;
    if (ly < 0) ly = uy; if (uy > result.height) uy = ly;
    if (outside(lx, ly) || outside(ux, uy)) return;
    
    var tl = getXY(lx, ly); var tr = getXY(lx+1, ly);
    var bl = getXY(lx, ly + 1); var br = getXY(lx + 1, ly + 1);
  ...
  function frame() {
    ...
    while (workingSet.length < maxAnimations && animationQueue.length > 0) {
      ...
      var line = Math.random() < 0.5 ? animationQueue.shift() : animationQueue.pop();
  ...
  function animate(line) {
    var from = line.idx
    var to = line.idx + Math.random() * pointsPerAnimation + pointsPerAnimation/2;
  ...
  function getColor(x, y) {
    ...
    var c = gradient(gray);
    return 'rgba(' + c.r +', ' + c.g + ',' + c.b + ', ' + (0.2 + gray * 1.5) +')';
  }
}
